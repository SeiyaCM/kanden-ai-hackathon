...
ローカルの最強推論環境（DGX Spark）を中心に、クラウド（A5000）で学習した知能をデプロイし、MCP経由で現実空間（Home Assistant）を制御するアーキテクチャです。
...
graph TD
    ...
    %% 推論環境（ローカル）
    subgraph LocalEnv ["GX Spark (Local AI Brain)"]
    ...
    %% 連携層
    subgraph BridgeLayer ["連携プロトコル"]
        MCP["🔌 MCP Server<br/>(イベント検知 & ツール実行)"]
    end
    ...
    %% 4. 現実世界の制御
    LLM -->|"⑤ ツール呼び出し (JSON)"| MCP
    MCP -->|"⑥ 機器制御API実行"| HA_Out
    
    HA_Out --> AC
    HA_Out --> Fan
    HA_Out --> Light

    %% 5. 学習とデプロイの流れ（バックグラウンド）
    SD -->|"4200枚Upload"| HF
