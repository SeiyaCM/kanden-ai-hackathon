...
- **`/lego`** — LEGO PF 赤外線送信。**POST** のみ。例: `ch=0&pwm=15&out=b`（青側・正転最大）、停止は `pwm=0`（詳細はファームウェアの `GET /lego` ヘルプ）
...
- **0.7 以上**: 固定文言の `GET /speech`（`curl --data-urlencode`）の後、`POST /lego` で `ch=0`・青側（`out=b`）を正転最大（`pwm=15`）し、**10 秒**の `delay` のあと `pwm=0` で停止。再入防止のため **`mode: single`**。
